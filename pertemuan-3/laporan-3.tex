...
Pada pertemuan pertama pengujian hanya berjalan di GitHub Actions karena laptop belum punya PHP. Untuk praktikum ini saya memasang PHP 8.5 lewat PPA \texttt{ondrej/php} beserta Composer, supaya perintah \texttt{php artisan test} bisa dijalankan di laptop seperti yang diminta pada langkah tugas. Saya memilih versi 8.5 agar sama dengan versi yang dipakai runner di workflow. Setelah pemasangan, pengujian dan pemeriksaan gaya kode dijalankan langsung di laptop:

\begin{lstlisting}[style=bashstyle,caption={Verifikasi lokal setelah PHP dipasang.}]
php -v
...
\begin{figure}[H]
    ...
    \caption{Hasil pengujian dan pemeriksaan gaya kode di laptop setelah PHP 8.5 dipasang.}
    \label{fig:lokal}
\end{figure}
