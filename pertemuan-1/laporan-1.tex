...
Dalam konteks konstruksi perangkat lunak, Git membantu menjaga setiap perubahan tetap terdokumentasi tanpa mengandalkan folder \texttt{final\_v2\_revisi} karena semua tercatat di riwayat commit. Ketika terjadi kesalahan, riwayat tersebut memudahkan pelacakan siapa mengubah apa dan kapan, sehingga keputusan untuk revert atau memperbaiki lewat commit baru dapat diambil secara terukur.

\section{Strategi Branching: \texttt{main} $\leftarrow$ \texttt{dev} $\leftarrow$ \texttt{feature/*}}
Strategi branching yang digunakan mengikuti pola sederhana sesuai instruksi tugas: \texttt{main} sebagai branch stabil, \texttt{dev} sebagai integrasi, dan \texttt{feature/*} untuk pekerjaan fitur. Alurnya \texttt{feature/*} $\rightarrow$ \texttt{dev} $\rightarrow$ \texttt{main} \cite{github-flow,drake2023}. Model ini merupakan penyederhanaan dari Git Flow \cite{atlassian-gitflow} yang memisahkan pekerjaan agar \texttt{main} tetap stabil.

Branch \texttt{dev} dibuat dari \texttt{main}, lalu setiap fitur dikerjakan di branch \texttt{feature/} yang bercabang dari \texttt{dev}. Setelah selesai, dibuka Pull Request \texttt{feature} $\rightarrow$ \texttt{dev}, kemudian setelah stabil dibuka PR \texttt{dev} $\rightarrow$ \texttt{main}. Dengan cara ini \texttt{main} hanya menerima perubahan yang sudah melewati review dan CI di \texttt{dev}.

\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-03-graph.jpg}}
    \caption{Ilustrasi alur branching \texttt{feature} $\rightarrow$ \texttt{dev} $\rightarrow$ \texttt{main} — setiap panah adalah Pull Request, bukan push langsung (graf \texttt{git log --graph} yang sama).}
    \label{fig:branching}
\end{figure}
...
Pada tugas ini semua commit dijaga menggunakan format tersebut dan commit polos seperti ``update'' yang dinilai nol dihindari. Contoh yang digunakan: \texttt{feat: tambah logika perhitungan IP semester berbobot SKS}, \texttt{test: tambah pengujian unit untuk bobot dan hitungIP}, \texttt{ci: tambah workflow dua job untuk uji unit dan lint style}. Pesan yang jelas memudahkan penilaian progres tanpa harus membuka tiap file.
...
Pada praktikum ini tidak ada push langsung ke \texttt{main} atau \texttt{dev} setelah protection aktif. Semua perubahan melalui PR. GitHub menampilkan status check dari Actions di dalam PR, sehingga merge baru dilakukan ketika kedua job CI hijau. Protection sengaja diuji dengan mencoba push langsung dan hasilnya ditolak dengan pesan \texttt{GH006: Protected branch update failed}, yang kemudian diperbaiki melalui branch \texttt{fix/revert-*} dan PR kembali.
...
Workflow yang disusun memiliki dua job paralel sesuai instruksi tugas:
\begin{itemize}
...
\end{itemize}
Keduanya berjalan di \texttt{ubuntu-latest} dengan PHP 8.5 (awalnya sempat gagal di 8.3 karena Laravel 13 butuh $\geq$8.4, lalu diperbaiki). Indikator hijau di tab Actions menjadi bukti bahwa perubahan layak di-merge.

\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-06-actions.jpg}}
    \caption{Konsep CI yang diterapkan: setiap push ke \texttt{main} atau \texttt{dev} memicu dua job paralel (\texttt{Uji unit \& fitur} dan \texttt{Lint style}) yang harus hijau sebelum merge (cuplikan Actions yang sama).}
    \label{fig:ci-konsep}
\end{figure}

\section{Branch Protection Rule}
Branch protection adalah aturan di GitHub yang mencegah push langsung dan mewajibkan status check sebelum merge \cite{github-protection}. Pada repository ini, \texttt{main} dan \texttt{dev} diproteksi dengan:
\begin{itemize}
...
\end{itemize}
Setelah protection aktif, setiap percobaan push langsung tertolak dan merge hanya bisa melalui PR yang sudah hijau. Dosen (\texttt{alesana001} / Galih Malela Damaraji) ditambahkan sebagai collaborator dengan hak \texttt{Read} agar dapat menilai tanpa mengubah kode.

\clearpage

\chapter[\tocboldsection{Hasil dan Pembahasan}]{Hasil dan Pembahasan}
Pada bab ini saya menjelaskan pekerjaan untuk Tugas 1 sesuai instruksi di Classroom: join organization \texttt{KEPL2026}, membuat repository publik \texttt{evolusi-pl-NIM}, menerapkan Conventional Commits, alur branch, dua PR, CI dua job, dan branch protection. Semua langkah diverifikasi dengan perintah \texttt{gh}, \texttt{git}, dan eksekusi lokal.
...
\begin{itemize}
    ...
    \item Repository: \texttt{hafidzrizqullahprasetya/evolusi-pl-535493} (publik, default \texttt{main}) sesuai NIM 535493 dan dapat diakses untuk penilaian.
...
Repository publik dibuat dengan nama \texttt{evolusi-pl-535493} sesuai NIM 535493. Repository dibuat publik agar kuota GitHub Actions gratis dan dapat diakses dosen. Perintah verifikasi yang dijalankan:
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-01-repo-view.jpg}}
...
Repository contoh \texttt{KEPL2026/evolusi-pl-contoh} tersedia sebagai referensi dan repository tugas ini saya kerjakan di akun personal dengan konfigurasi yang sama sesuai instruksi.

\section{Penerapan Conventional Commits}

Semua commit dijaga mengikuti Conventional Commits. Tidak ada commit ``update'' polos. Riwayat commit yang dihasilkan (tanpa merge):
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-02-commits.jpg}}
...
\texttt{dev} dibuat dari \texttt{main}, lalu setiap fitur dari \texttt{dev}:
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-04-branch.jpg}}
...
Tidak ada push langsung ke branch terproteksi setelah aturan aktif. Semua perubahan harus melalui PR.
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-05-pr-list.jpg}}
...
Setiap PR menampilkan checks dari GitHub Actions. Merge baru dilakukan setelah kedua job hijau sesuai aturan protection.

\section{Pipeline CI Dua Job}

File \texttt{.github/workflows/ci.yml} ditambahkan dengan dua job paralel:
...
Awalnya job sempat gagal di PHP 8.3 karena Laravel 13 membutuhkan PHP $\geq$8.4 (error Symfony 8.1), lalu diperbaiki ke 8.5 di commit \texttt{ci: ganti PHP runner ke 8.5} dan sejak itu semua hijau:
...
Branch protection pada \texttt{main} dan \texttt{dev} diaktifkan lewat API. Hasil verifikasi:
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-07-protection.jpg}}
...
Untuk membuktikan protection bekerja, sengaja dilakukan percobaan push langsung ke branch terproteksi:
...
Setelah itu perbaikan dilakukan lewat branch \texttt{fix/revert-*} dan merge via PR seperti yang terlihat di graf sebelumnya. Protection kemudian dikembalikan ke \texttt{enforce\_admins: true} setelah pengujian.

Untuk collaborator, dosen ditambahkan sebagai Read sesuai instruksi:
...
Akun \texttt{alesana001} (Galih Malela Damaraji) telah di-invite dengan hak \texttt{pull} (Read). Di daftar collaborator, invite tercatat dan menunggu diterima. Hak Write/Admin tidak diberikan agar sesuai instruksi ``peran Read''.

\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-08-collab.jpg}}
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-09-phpunit-pint.jpg}}
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.95\textwidth]{images/ss-10-ip-page.jpg}}
...
\chapter{Kesimpulan}
Berdasarkan praktikum pada Pertemuan 1, dapat disimpulkan beberapa hal:

\begin{enumerate}
    \item Praktik manajemen versi dengan Git dan GitHub telah diterapkan. Repository \texttt{evolusi-pl-535493} dibuat publik dengan default branch \texttt{main} dan semua perubahan tercatat sebagai commit yang dapat ditelusuri.
    \item Penerapan Conventional Commits dilakukan untuk 9 commit inti tanpa ada commit ``update'' polos, serta alur branching \texttt{feature/*} $\rightarrow$ \texttt{dev} $\rightarrow$ \texttt{main} dijaga tetap bersih. Semua perubahan masuk melalui Pull Request, bukan push langsung.
    \item Pipeline CI dengan dua job paralel (\texttt{Uji unit \& fitur} dan \texttt{Lint style (Pint)}) di GitHub Actions disusun dan berjalan hijau untuk setiap push dan pull request ke \texttt{main} dan \texttt{dev}. Perbaikan versi PHP dari 8.3 ke 8.5 menjadi contoh bagaimana CI memberi umpan balik cepat.
    \item Branch protection pada \texttt{main} dan \texttt{dev} dikonfigurasi dengan \texttt{enforce\_admins: true}, \texttt{strict: true}, dan dua required checks, serta telah diuji bahwa push langsung memang tertolak. Dosen (\texttt{alesana001}) juga telah diundang sebagai collaborator Read sesuai instruksi.
\end{enumerate}

Dengan ini, alur evolusi perangkat lunak yang diminta di Tugas 1 sudah berjalan: kode teruji (unit test lolos), konsistensi gaya kode terjaga (lint Pint lolos), riwayat commit rapi, dan branch utama terlindungi.
