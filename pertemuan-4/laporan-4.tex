...
\begin{tabular}{@{}l l @{}}
  ...
  \textbf{Kelas}          & : PL5A1 \\
  \textbf{Dosen Pengampu} & : Galih Malela Damara\v{z}i \\
\end{tabular} \\[1.2cm]
...
CORS mengatur izin akses lintas domain di peramban \cite{mdn-cors}. Ketika aplikasi Vue dijalankan di \texttt{localhost:5173} dan API berada di \texttt{127.0.0.1:8001}, keduanya dianggap domain berbeda. Laravel menangani hal ini lewat berkas \texttt{config/cors.php} yang menentukan jalur, metode, dan asal yang diizinkan \cite{laravel-cors}.
...
Ada beberapa kendala yang saya temui. Pengujian Vitest tidak boleh bergantung pada backend, sehingga logika yang diuji harus saya pisahkan dulu ke berkas tersendiri; kalau tidak, test akan gagal di GitHub Actions karena Laravel tidak berjalan di sana. Job \texttt{deploy} juga tetap memerlukan direktori \texttt{dist} padahal job itu tidak menjalankan build, jadi hasil build harus dioper lewat artifact. Jalur cache \texttt{npm} pun perlu diarahkan ke \texttt{frontend/package-lock.json} karena berkas kunci berada di subfolder, bukan di akar repository.
